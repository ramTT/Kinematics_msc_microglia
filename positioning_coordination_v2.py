...
#D. Melting dataset
del instance_position.data_frame['side']
data_frame_boostrap = instance_position.data_frame.melt(id_vars = ['RH.index', 'day', 'group', 'force', 'displacement'])

#Displacement and force adjustors
def value_normalizer(column_name):
    divisor = min(data_frame_boostrap[column_name])
    data_frame_boostrap[column_name+'_index'] = data_frame_boostrap[column_name].map(lambda value: value/divisor)

value_normalizer('force')
value_normalizer('displacement')

#Separating joint and coordinate information
...
data_frame_boostrap.drop(['variable', 'force', 'displacement'], axis=1, inplace=True)

#Adjusting value for force or displacement
logical_subset = ~(data_frame_boostrap['day']==3)
data_frame_boostrap.loc[logical_subset, 'value'] = data_frame_boostrap.loc[logical_subset, 'value']*data_frame_boostrap.loc[logical_subset,'displacement_index']
data_frame_boostrap.drop(['force_index', 'displacement_index'], axis=1, inplace=True)

#################################################################### BOOTSTRAPPING DATA FOR PLOTTING ##############################################################
#A. Bootstrapping data from each animal/day/variable
data_frame_boostrap = dict(list(data_frame_boostrap.groupby(['joint_type', 'coord_type', 'RH.index', 'day'])))
...
data_frame_boostrap_aggregate.columns = ['RH.index', 'day', 'joint_name',
                                         'mean_x', 'std_x',
                                         'mean_y', 'std_y',
                                         ]
...
data_frame_boostrap_summary = data_frame_boostrap_aggregate.groupby(['day', 'group', 'joint_name'],
    as_index=False).agg({'mean_x':'mean', 'mean_y':'mean',
                         'std_x':lambda std_dev:mean_of_std(std_dev),
...
def coordinates_overtime_plot_extended_v2(data_technical, data_biological, data_summary, plot_day, study_group):
    ...
    plt.xticks(list(np.arange(0, 4, 0.5)))
    plt.yticks(list(np.arange(0, 4.5, 0.5)))
    #plt.xlim([-0.5, 4])
    plt.title('Day Post SCI:'+' '+str(plot_day), size=15, fontweight='bold')
    plt.scatter(0.5, 4.5, s=100, color=group_2_color('sci'))
    plt.scatter(1, 4.5, s=100, color=group_2_color('sci_medium'))
    plt.scatter(1.8, 4.5, s=100, color=group_2_color('sci_msc'))
    plt.annotate('SCI', fontweight='bold', size=12, xy=(1, 1), xytext=(0.55, 4.475))
    plt.annotate('SCI+Medium', fontweight='bold', size=12, xy=(1, 1), xytext=(1.05, 4.475))
    plt.annotate('SCI+Medium+MSCs', fontweight='bold', size=12, xy=(1, 1), xytext=(1.85, 4.475))
