...
class DataImportAdjust:

    def __init__(self, raw_data_set):
        self.raw_data_set = raw_data_set

    def data_subset_rename(self, select_columns, select_column_names):
        """Selects relevant columns and renames appropriately"""
        clean_data = self.raw_data_set.loc[:, select_columns]
        clean_data = clean_data.rename(columns=select_column_names)

        return clean_data

    def add_group(self, data_set_main, data_set_key):
    ...
    def data_aggregation_adjuster(self, data_not_aggregated):
        ...
        while counter < number_of_indices:
            dataframe_new[data_not_aggregated.index.names[counter]] = data_not_aggregated.index.get_level_values(
                level=counter)
            counter += 1
...
#Creating instances of DataImportAdjust class
side_view_instance = DataImportAdjust(data_side_view_raw)
bottom_view_instance = DataImportAdjust(data_bottom_view_raw)
...
data_side = side_view_instance.data_subset_rename(side_view_columns, side_view_column_names)
data_bottom = bottom_view_instance.data_subset_rename(bottom_view_columns, bottom_view_column_names)
data_angle = side_view_instance.data_subset_rename(angle_columns, angle_column_names)

#Merging datasets with data_key
data_side = side_view_instance.add_group(data_side, data_key)
data_bottom = bottom_view_instance.add_group(data_bottom, data_key)
data_angle = side_view_instance.add_group(data_angle, data_key)
...
data_angle_aggregated_export = data_angle_aggregated.copy()
